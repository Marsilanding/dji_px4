...
int dji_authority = 0; /*flag de autoridad de control*/
...
void dji_control_cb(const sensor_msgs::Joy::ConstPtr& msg){
    joy_command_rec = *msg;
    joy_x_rec.data = joy_command_rec.axes[0];
    joy_y_rec.data = joy_command_rec.axes[1];
    joy_z_rec.data = joy_command_rec.axes[2];
...
bool ctrl_request(
       dji_px4::SDKControlAuthority::Request &req,
       dji_px4::SDKControlAuthority::Response &resp){
       dji_authority = req.control_enable;
       ros::param::set("/dji_client/control_authority", dji_authority);
       ROS_INFO("Comando SetControlAuthority --%d recibido", dji_authority);
       return true;
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "dji_px4_interface");
    ros::NodeHandle nh;
    ...
    ros::ServiceServer task_server = nh.advertiseService("/dji_sdk/drone_task_control",&task_request);

    ros::ServiceServer auth_server = nh.advertiseService("/dji_sdk/sdk_control_authority",&ctrl_request);
    ...
    ros::Subscriber local_pos_sub = nh.subscribe<geometry_msgs::PoseStamped> /*Local Position*/
     ...
     ros::Subscriber odom_sub = nh.subscribe<nav_msgs::Odometry> /*Odometry*/
            ("ual/odom", 10, odom_cb);
    ...
    ros::Subscriber dji_enuposition_yaw_sub = nh.subscribe<sensor_msgs::Joy>
        ("/dji_sdk/flight_control_setpoint_ENUposition_yaw", 10, dji_control_cb);

    ros::Subscriber dji_enuvelocity_yawrate_sub = nh.subscribe<sensor_msgs::Joy>
        ("/dji_sdk/flight_control_setpoint_ENUvelocity_yawrate", 10,  dji_control_cb);

    ros::Subscriber dji_rollpitch_yawrate_zpos_sub = nh.subscribe<sensor_msgs::Joy>
        ("/dji_sdk/flight_control_setpoint_rollpitch_yawrate_zposition", 10, dji_control_cb);
    ...
    dji_px4::SDKControlAuthority::Request dji_authority_req;
    dji_px4::SDKControlAuthority::Response dji_authority_resp;
    ...
    while(ros::ok()){
        ...
        nh.getParam("/dji_client/control_authority", dji_authority);
        nh.getParam("/dji_client/control_mode", ctrl_flag);

        if(dji_authority == 0){
            ROS_INFO("Control no autorizado...");
        }
        else{
            if(ctrl_flag == 0){
            ...
            if(dji_task == 4){
                ros::param::get("/dji_client/takeoff_height", takeoff_height);
                ros::param::get("/dji_client/blocking", blocking);
